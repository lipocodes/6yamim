...
class FirebaseMethods /*implements AppMethods*/ {
  ...
  createUserAccount({
    ...
    try {
      ...
      await firestore.collection(usersData).document(user.user.uid).setData({
  ...
  removeOrder(String orderID, int index, String email) async {
    String userEmail = await getStringDataLocally(key: "userEmail");
    String shopID = await getStringDataLocally(key: "shopID");
    ...
    try {
      await firestore.collection('orders_' + shopID).document(orderID).delete();
    } on PlatformException catch (e) {}
